...
for i in range(2):  # two class
    u.append(np.mean(X[y == i], axis=0))  # column mean
print "np.mean: ", np.mean(X[y == i], axis=0)
"""
...
# n is the number of attributes of one sample
m, n = np.shape(X)

# print m, n
Sw = np.zeros((n, n))
for i in range(m):
    # X[i] is a row vector, row i of X
...
# print "UEVT: ", np.dot(np.dot(U, sigma), VT)
# print "U: \n", U
# print "sigma: \n", sigma
# print "VT: \n", VT
# print "u: \n", u
...
# w.shape is (2, 1)
# slope of the line is (w[1,0]/w[0,0])
# p0_x0 = -X[:, 0].max()
p0_x0 = -1000
